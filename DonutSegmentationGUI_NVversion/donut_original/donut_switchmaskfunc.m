...
if gh.data.MaskType(1,IdxMin)==1
    ...
    OverlapMask=((gh.data.MaskPatchFin{1,IdxMin}.*imfill(gh.data.LblMaskM(xL:xR,yL:yR)))==0);
...
elseif gh.data.MaskType(1,IdxMin)==2
    ...
    if get(gh.main.PopupMenuObjType,'value')==1
        if (size(gh.data.MaskPatchFin{1,IdxMin},1)==(2*gh.param.ModelHlfWid+1)) && (size(gh.data.MaskPatchFin{1,IdxMin},2)==(2*gh.param.ModelHlfWid+1))
            gh.data.MaskPatchFin{1,IdxMin}=gh.data.MaskPatchFin{1,IdxMin}.*R_Circ1;
            [gh.data.MaskPatchFin{1,IdxMin}]=donut_convHullConnect(gh.data.MaskPatchFin{1,IdxMin},IdxMin);
        end
    end
    
    OverlapMask=((gh.data.MaskPatchFin{1,IdxMin}.*imfill(gh.data.LblMaskI(xL:xR,yL:yR)))==0);
