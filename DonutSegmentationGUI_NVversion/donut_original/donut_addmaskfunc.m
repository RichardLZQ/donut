...
gh.data.ix(end+1)=NewMaskP(1)/gh.param.ImSclFact;
gh.data.iy(end+1)=NewMaskP(2)/gh.param.ImSclFact;
...
if isempty(gh.data.LblMask==size(gh.data.ix,1))
    ...
    gh.data.InitLblMask(NewMaskP(1),NewMaskP(2))=size(gh.data.ix,1);
