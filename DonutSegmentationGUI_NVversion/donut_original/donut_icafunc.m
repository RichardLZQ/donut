...
for ii=StartNum:size(gh.data.ix,1) % for each ROI
    ...
    if ~isempty(gh.data.Ma{1,ii})
        ...
        M1=mean(gh.data.Ma{1,ii}(:,1)); % pix x 2 ICA > mean loading on first ICA
        ...
        if (sum(gh.data.MaskPatchBin{1,ii}(:))<AreaMax)...
            ...
            if get(gh.main.PopupMenuObjType,'value')==1
                if (size(gh.data.MaskPatchFin{1,ii},1)==(2*gh.param.ModelHlfWid+1)) && (size(gh.data.MaskPatchFin{1,ii},2)==(2*gh.param.ModelHlfWid+1))
                    gh.data.MaskPatchFin{1,ii}=gh.data.MaskPatchFin{1,ii}.*R_Circ1;
                    [gh.data.MaskPatchFin{1,ii}]=donut_convHullConnect(gh.data.MaskPatchFin{1,ii},ii);
                end
            end
            OverlapMask=((gh.data.MaskPatchFin{1,ii}.*imfill(gh.data.LblMaskI(xL:xR,yL:yR)))==0);
    ...
    if ~gh.data.MaskType(1,ii)
        ...
        if (sum(gh.data.MaskPatchBin{1,ii}(:))<AreaMax)...
            ...
            OverlapMask=((gh.data.MaskPatchFin{1,ii}.*imfill(gh.data.LblMaskM(xL:xR,yL:yR)))==0);
...
gh.data.LblMaskM=gh.data.LblMaskM.*(~imfill(gh.data.LblMaskI));
...
gh.data.LblMaskI=donut_breakROIfunc(gh.data.LblMaskI,gh.data.LblMaskI+gh.data.LblMaskM);
gh.data.LblMaskM=donut_breakROIfunc(gh.data.LblMaskM,gh.data.LblMaskI+gh.data.LblMaskM);
